(() => {
  'use strict';
  // Lightweight enhancement layer: timer, dynamic vitals, decision analytics and richer debrief.
  let timer = null, seconds = 35, decisions = 0, correct = 0, start = 0;
  ...
  function addUI(){
    ...
    const visual = $('visual'); if(main && visual) main.insertBefore(monitor, visual);
  }
  function setVitals(){
    const h=$('liveHR'), b=$('liveBP'), s=$('liveSpO2'); if(!h)return;
    const n=location.hash + ($('caseTitle')?.textContent||'');
  ...
  function observe(){
    ...
    new MutationObserver(()=>{
      ...
      buttons.forEach(btn=>{if(btn.dataset.eduBound)return; btn.dataset.eduBound='1';btn.addEventListener('click',()=>{
        decisions++; const wasCorrect=btn.classList.contains('correct'); if(wasCorrect)correct++;
        const acc=Math.round(correct/decisions*100); if($('eduAccuracy'))$('eduAccuracy').textContent=acc+'%';
  ...
  function loop(){
    ...
    if(game() && !$('eduTimer')?.dataset.running){tick(); if($('eduTimer'))$('eduTimer').dataset.running='1';}
