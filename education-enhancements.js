(() => {
  ...
  function effectFor(choice,isCorrect){
    const c=(choice||'').toLowerCase(), type=caseType();
    ...
    if(type==='ectopic'){
      ...
      if(/magnesium|مغنيسيوم|seizure|اختلاج/.test(c)) e={health:7,hr:-2,sbp:-4,dbp:-3,spo2:.2};
      if(/urgent|delivery|ولادة|senior|anaesthetic|critical|severe|تصعيد|labetalol|nifedipine|hydralazine/.test(c)) e={health:5,hr:-3,sbp:-5,dbp:-4,spo2:.2};
      if(/wait|discharge|routine|home|انتظار|منزل/.test(c)) e={health:-12,hr:8,sbp:8,dbp:6,spo2:-.8};
    } else if(type==='pph'){
      if(/call|help|team|uter|massage|oxytocin|uterotonic|iv|blood|resusc|نزف|إنعاش|فريق|tranexamic|txa/.test(c)) e={health:8,hr:-7,sbp:7,dbp:5,spo2:.5};
      ...
      if(/mcc?robert|suprapubic|فوق العانة|help|call|مساعدة|فريق/.test(c)) e={health:7,hr:-2,sbp:1,dbp:1,spo2:.2};
      if(/fundal|شد|traction|pull|ضغط قاع/.test(c)) e={health:-12,hr:6,sbp:-4,dbp:-2,spo2:-.5};
    }
  ...
  function classifySkill(choice,isCorrect,elapsed){
    ...
    const unsafe=/(wait|delay|discharge|home|routine|fundal|traction|pull|methotrexate)/.test(c);
    if(isCorrect&&recognition)skill.recognition+=1;
    if(isCorrect&&priority)skill.prioritization+=1;
    if(!unsafe&&isCorrect)skill.safety+=1;
    if(elapsed<=10&&isCorrect)skill.prioritization+=0.25;
    void type;
  }
  ...
  function tick(){
    clearInterval(timer);seconds=35;start=Date.now();const t=$('eduTimer');if(!t)return;
    t.parentElement.classList.remove('danger');t.textContent=seconds;if($('eduPace'))$('eduPace').textContent='Ready';
    timer=setInterval(()=>{if(!game()){clearInterval(timer);return;}seconds--;t.textContent=Math.max(0,seconds);if(seconds<=10&&seconds>0)t.parentElement.classList.add('danger');if(seconds<=0){clearInterval(timer);t.parentElement.classList.add('danger');if($('eduPace'))$('eduPace').textContent='TIME';}},1000);
  }
  ...
  function observe(){
    const choices=$('choices');if(!choices||choices.dataset.eduObserved)return;choices.dataset.eduObserved='1';
    new MutationObserver(()=>[...choices.querySelectorAll('button')].forEach(btn=>{if(btn.dataset.eduBound)return;btn.dataset.eduBound='1';btn.addEventListener('click',()=>{
      decisions++;const isCorrect=btn.classList.contains('correct');if(isCorrect)correct++;const elapsed=Math.round((Date.now()-start)/1000);
      classifySkill(btn.textContent,isCorrect,elapsed);applyEffect(btn.textContent,isCorrect);
      if($('eduAccuracy'))$('eduAccuracy').textContent=Math.round(correct/decisions*100)+'%';if($('eduPace'))$('eduPace').textContent=elapsed<=10?'FAST':elapsed<=20?'GOOD':'SLOW';
  ...
  function report(){
    ...
    const speed=decisions?Math.min(100,Math.round((skill.prioritization/decisions)*100)):0;
  ...
  function loop(){
    ...
    if(game()){
      ...
      if(ck&&ck!==currentCaseKey){currentCaseKey=ck;lastStepKey='';decisions=0;correct=0;skill={recognition:0,prioritization:0,safety:0};if($('eduAccuracy'))$('eduAccuracy').textContent='0%';resetPatient();}
