(() => {
  'use strict';
  let timer = null, seconds = 35, decisions = 0, correct = 0, start = 0, lastCase = '';
  ...
  function setVitals(){
    ...
    let v=/Ectopic|الحمل الهاجر/i.test(n)?['128','82/50','97%']:/Preeclampsia|مقدمات/i.test(n)?['104','170/112','98%']:/Haemorrhage|نزف|PPH/i.test(n)?['122','88/54','95%']:['92','118/72','98%'];
    h.textContent=v[0];b.textContent=v[1];s.textContent=v[2];
  }
  function tick(){
    ...
    timer=setInterval(()=>{if(!game()){clearInterval(timer);return;}seconds--;t.textContent=Math.max(0,seconds);if(seconds<=0){clearInterval(timer);t.parentElement.classList.add('danger');}},1000);
  }
  function observe(){
    const choices=$('choices'); if(!choices||choices.dataset.eduObserved)return;
    choices.dataset.eduObserved='1';
    new MutationObserver(()=>{
      [...choices.querySelectorAll('button')].forEach(btn=>{if(btn.dataset.eduBound)return;btn.dataset.eduBound='1';btn.addEventListener('click',()=>{
        decisions++;const wasCorrect=btn.classList.contains('correct');if(wasCorrect)correct++;const acc=Math.round(correct/decisions*100);
        if($('eduAccuracy'))$('eduAccuracy').textContent=acc+'%';const pace=Math.round((Date.now()-start)/1000);if($('eduPace'))$('eduPace').textContent=pace<=10?'FAST':pace<=20?'GOOD':'SLOW';setTimeout(setVitals,120);
      });});
    }).observe(choices,{childList:true});
  }
  function enrichEnd(){
    const end=$('end');if(!end||end.dataset.eduEnhanced)return;end.dataset.eduEnhanced='1';
    const box=document.createElement('div');box.className='debriefBox';box.innerHTML='<h3>🧠 Clinical Debrief</h3><p>Strong emergency decision-making means recognizing deterioration early, prioritizing ABCs, reassessing after every intervention, escalating to the right team and avoiding dangerous delay.</p><div class="debriefGrid"><div><span>Decision accuracy</span><b id="finalAccuracy">0%</b></div><div><span>Response style</span><b id="finalPace">—</b></div><div><span>Clinical focus</span><b>Recognition + escalation</b></div></div><p class="pearl"><b>💡 Clinical Pearl:</b> Stabilize first when unstable, reassess continuously, and use local guidelines/protocols for definitive management.</p>';
    const ref=$('clinicalRefs');if(ref)end.insertBefore(box,ref);else end.insertBefore(box,end.firstChild);
  }
  function loop(){
    addUI();observe();setVitals();enrichEnd();
    if(game()){
      const current=$('caseTitle')?.textContent||'';
      if(current&&current!==lastCase){lastCase=current;tick();if($('eduAccuracy'))$('eduAccuracy').textContent=decisions?Math.round(correct/decisions*100)+'%':'0%';}
    } else clearInterval(timer);
    if($('finalAccuracy'))$('finalAccuracy').textContent=(decisions?Math.round(correct/decisions*100):0)+'%';
    if($('finalPace'))$('finalPace').textContent=seconds>20?'Calm & fast':'Focused under pressure';
